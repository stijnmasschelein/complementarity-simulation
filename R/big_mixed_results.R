...
# Save big tbl ----
start <- Sys.time()
saveRDS(dat, file = "simulated_data/big_mixed_simulation.RDS")
end <- Sys.time()
print(end - start)
...
mutate(stat_type = if_else(grepl("c\\(0, 0, 0\\)", b2_str), 
                              "type~I", "power")) %>%
...
start <- Sys.time()
saveRDS(summ, "simulated_data/mixed_simulation_summary.RDS")
end <- Sys.time()
print(end - start)
