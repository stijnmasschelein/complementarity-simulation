...
var1 = function(interval = c(-5, 5), b12 = 0, d = c(1, 1), 
                g = c(.5, .5), s = c(.5, .5)){
  varx2 = (interval[2] - interval[1])^2 / b12;
  varz2 = (d[2] * g[1] + b12 * g[2])/(d[1] * d[2] - b12^2);
  return(varx2 + varz2)
}

var2 = function(interval = c(-5, 5), b12 = 0, d = c(1, 1), 
                g = c(.5, .5), s = c(s1, s2)){
  vare = (d[2]^2 * s[1]^2 + b12^2 * s[2]^2)/(d[1] * d[2] - b12^2)^2
  return(vare)
}

#' # Calculate the optimal level

calc_opt = function(b1 = c(0, 0), b12 = 0, d = c(1, 1), 
                    g = c(.5, .5), z = 0){
  x_star = (d[2] * b1[1] + b12 * b1[2] +  
              (d[2] * g[1] + b12 * g[2]) * z) / (d[1] * d[2] - b12^2)
  return(x_star)
}

#' # A single simulation

sim_vars = function(interval = c(-5, 5), obs = 200, rep = 1, 
                    rate = 0.5, sd = 1,
